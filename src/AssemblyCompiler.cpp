...
AssemblyCompiler::AssemblyCompiler(const std::string &filePath) {
...
void AssemblyCompiler::writeIntoABinFile(const std::string &filePath) {
    ...
    for (unsigned int &i: binCode) {
        fs.write((char *) &i, sizeof(uint32_t));
    }
    fs.close();
}
...
void AssemblyCompiler::readFromFile(const std::string &filePath) {
    std::ifstream file;
    file.open(filePath);
    if (!file.is_open()) {
...
void AssemblyCompiler::ConstsAndLabels(const std::string &str, int line) {
    ...
    if (found != std::string::npos) {
        ...
        if (!isNumber(number) ||
            consts.find(name) != consts.end())//check if const val is a number and is not being redifined
        {
...
void AssemblyCompiler::parseInstruction(std::string &instruction, int line) {
    ...
    std::string instr;
    uint32_t code = 0;
    size_t k1 = instruction.find_first_not_of(' ', 0);
    size_t k2 = instruction.find_first_of(' ', k1 + 1);
